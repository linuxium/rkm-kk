...
static int rtl8152_suspend(struct usb_interface *intf, pm_message_t message)
{
	...
	if ((message.event & PM_EVENT_AUTO) != 0)
...
static int __init rtl8152_init(void)
{
	int retval = usb_register(&rtl8152_driver);
	if( retval )
		err("usb_register failed. Error number %d", retval);
	return retval;
}

static void __exit rtl8152_exit(void)
{
	usb_deregister(&rtl8152_driver);
}

module_init(rtl8152_init);
module_exit(rtl8152_exit);
